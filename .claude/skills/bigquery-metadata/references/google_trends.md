...
## Dimensions
- **Geography**: `dma_name` / `dma_id` (US only) vs `country_code` / `region_code` (international). The same idea, different columns per table.
- **Time**: `week` (the ranking's week, a Sunday) vs `refresh_date` (when Google published it — **the partition**). These are different; see Gotchas.
- **Rank vs score**: `rank` 1–25 (1 = top); `score` 0–100 relative interest; `percent_gain` (rising tables only) = week-over-week growth.
